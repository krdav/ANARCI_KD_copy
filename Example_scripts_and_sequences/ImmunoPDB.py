...
from anarci import anarci, scheme_names
...
class PDBNumber(object):
    ...
    '''
    def __init__(self,scheme,allowed_domains, region_definition=None):
        self.scheme=scheme
...
class AntigenReceptorPDBParser(PDBParser):
    ...
    def __init__( self, PERMISSIVE=True, get_header=False,
                 structure_builder=None, QUIET=False, scheme='imgt', region_definition='imgt'):
        '''
        '''
    ...
    def get_structure(self, id, file):
        """Return the structure. Variable domains are annotated with the chose numbering scheme.
        ...
        o The xtra['pairing'] list of a chain tells you how the variable domains it contains are paired. 
            e.g. For a normal Fv with chains H and L. 
                structure[0]['H'].xtra['pairing']  -->    [ (<Chain id=H>, 0) ]
                structure[0]['L'].xtra['pairing']  -->    [ (<Chain id=L>, 0) ]
        ...
        o Residues after the variable domain are numbered sequentially from 1001. No guarntee is given of structural equivalence in 
        ...
        structure = super( AntigenReceptorPDBParser, self ).get_structure(id, file)
        seqres    = self.read_seqres( file )
        self.number_receptor_chains(structure, sequences=seqres) # Post process
    ...
    def read_seqres(self, file):
        ...
        '''
        try:
            seqres   =  dict([ (a.name.split(':')[1], str(a.seq)) for a in PdbSeqresIterator(open(file))])
        except:
            seqres = {}
        return seqres
    ...
    def number_receptor_chains(self, structure, sequences={}):
        ...
        '''
        pdbN = PDBNumber(self.scheme, self.allowed_domains, region_definition=self.region_definition)
    ...
    for model in structure:
        ...
        for chain in chains: 
            if len( chain.xtra.get('domains',[]) ) == 2 and chain.id.lower() not in model: 
                ...
                if c.id == model.xtra['firstpair'][0][0].id:
